...
def parse(line: str, line_number: int, code_addr: int) -> list[str]:
    binary = []

    # Break down the line of code
    line = replace_tabs(line.strip().upper()).split()
    ...
    inst = find_inst(line[0])
    if inst == -1:
        if not line[0].endswith(':'):
            raise ValueError(f"Error at line {line_number}: Invalid instruction.")
        elif line[0][:-1] in label.keys():
            raise ValueError(f"Error at line {line_number}: The label is already declared.")
        else:
    ...
    for i in range(3):
        if len(line) <= i + 1 or not line[i + 1]:
            ...
            if reg != -1:
                ...
                raise ValueError(f"Error at line {line_number}: Invalid register or undeclared label.")
    ...
    try:
        with open(input_file, "r") as inf, open(output_file, "w") as outf:
            for line_number, line in enumerate(inf, start=1):
                try:
                    binary_line = parse(line, line_number, code_addr)
                    if binary_line:
